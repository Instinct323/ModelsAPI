...
class GraspEstimator:
    ...
    def __init__(self, cfg):
        ...
        self._num_input_points = self._contact_grasp_cfg['DATA']['raw_num_points'] if 'raw_num_points' in self._contact_grasp_cfg['DATA'] else \
    ...
    def build_network(self):
        ...
        print("--- Get model")
        # Get model
        end_points = self._model_func.get_model(self.placeholders['pointclouds_pl'], self.placeholders['is_training_pl'], global_config,
                                                bn_decay=bn_decay)

        tf_bin_vals = self._model_func.get_bin_vals(global_config)
        offset_bin_pred_vals = tf.gather_nd(tf_bin_vals, tf.expand_dims(tf.argmax(end_points['grasp_offset_head'], axis=2), axis=2)) if \
            global_config['MODEL']['bin_offsets'] else end_points['grasp_offset_pred'][:, :, 0]

        grasp_preds = self._model_func.build_6d_grasp(end_points['approach_dir_head'], end_points['grasp_dir_head'], end_points['pred_points'],
                                                      offset_bin_pred_vals, use_tf=True)  # b x num_point x 4 x 4
        ...
        self.model_ops = {'pointclouds_pl': self.placeholders['pointclouds_pl'],
                          ...
                          'offset_pred_idcs_pc': tf.argmax(end_points['grasp_offset_head'], axis=2) if global_config['MODEL'][
                              'bin_offsets'] else None,
        ...
        for i in pc_segments:
            ...
            if np.any(pc_segments[i]):
                ...
                if np.any(partial_pc):
                    partial_pc = regularize_pc_point_count(partial_pc, self._contact_grasp_cfg['DATA']['raw_num_points'],
    ...
    def predict_grasps(self, pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        ...
        pc, pc_mean = preprocess_pc_for_inference(pc.squeeze(), self._num_input_points, return_mean=True,
        ...
        if constant_offset:
            offset_pred = np.array([[self._contact_grasp_cfg['DATA']['gripper_width'] - self._contact_grasp_cfg['TEST']['extra_opening']] *
                                    self._contact_grasp_cfg['DATA']['num_point']])

        gripper_openings = np.minimum(offset_pred + self._contact_grasp_cfg['TEST']['extra_opening'],
        ...
        selection_idcs = self.select_grasps(pred_points[:, :3], pred_scores,
                                            ...
                                            self._contact_grasp_cfg['TEST']['second_thres'] if 'second_thres' in self._contact_grasp_cfg['TEST'] else
                                            self._contact_grasp_cfg['TEST']['first_thres'],
        ...
        return pred_grasps_cam[selection_idcs], pred_scores[selection_idcs], pred_points[selection_idcs].squeeze(), gripper_openings[
            selection_idcs].squeeze()
    ...
    def predict_scene_grasps(self, pc_full, pc_segments={}, local_regions=False, filter_grasps=False, forward_passes=1,
        ...
        if local_regions:
            ...
            for k, pc_region in pc_regions.items():
                pred_grasps_cam[k], scores[k], contact_pts[k], gripper_openings[k] = self.predict_grasps(pc_region, convert_cam_coords=True,
                                                                                                         forward_passes=forward_passes)
        else:
            # print('using full pc')
            pc_full = regularize_pc_point_count(pc_full, self._contact_grasp_cfg['DATA']['raw_num_points'])
            pred_grasps_cam[-1], scores[-1], contact_pts[-1], gripper_openings[-1] = self.predict_grasps(pc_full, convert_cam_coords=True,
                                                                                                         forward_passes=forward_passes)
        ...
        if filter_grasps:
            ...
            for k in segment_keys:
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    ...
                    print('skipping obj {} since  np.any(pc_segments[k]) {} and np.any(contact_pts[j]) is {}'.format(k, np.any(pc_segments[k]),
    ...
    def select_grasps(self, contact_pts, contact_conf, max_farthest_points=150, num_grasps=200, first_thres=0.25, second_thres=0.2,
        ...
        _, center_indexes = farthest_points(contact_pts[conf_idcs_greater_than, :3], np.minimum(max_farthest_points, len(conf_idcs_greater_than)),
        ...
        if segmap is not None:
            ...
            for i in obj_instances:
                if skip_border_objects and not i == segmap_id:
                    ...
                    if np.any(obj_i_x < margin_px) or np.any(obj_i_x > segmap.shape[1] - margin_px) or np.any(obj_i_y < margin_px) or np.any(
                            obj_i_y > segmap.shape[0] - margin_px):
    ...
    def predict_scene_grasps_from_depth_K_and_2d_seg(self, sess, depth, segmap, K, z_range=[0.2, 1.8], local_regions=False, filter_grasps=False,
                                                     segmap_id=0, skip_border_objects=False, margin_px=5, rgb=None, forward_passes=1):
        """ Combines converting to point cloud(s) and predicting scene grasps into one function """

        pc_full, pc_segments = self.extract_point_clouds(depth, K, segmap=segmap, segmap_id=segmap_id, skip_border_objects=skip_border_objects,
                                                         margin_px=margin_px, z_range=z_range, rgb=rgb)
