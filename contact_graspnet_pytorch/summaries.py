...
def build_summary_ops(ops, sess, global_config):
    ...
    if global_config['MODEL']['bin_offsets']:
        ...
        tf.summary.scalar('offset_bin_classification_accuracy',
                          tf.reduce_mean(tf.cast(tf.equal(ops['offset_pred_idcs_pc'], ops['offset_label_idcs_pc']), tf.float32)))
    ...
    summary_ops = {'merged': merged, 'merged_eval': merged_eval, 'pr_update_op': pr_update_op, 'auc_update_op': auc_update_op,
                   'acc_update_ops': acc_update_ops, 'pr_reset_op': pr_reset_op}
    return summary_ops
